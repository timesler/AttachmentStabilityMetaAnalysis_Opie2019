...
dataRaw$Risk <- dataRaw$Medical.Risk == 1 | dataRaw$Social.Risk == 1
  ...
  for (moderator in moderators)
  {
    if (fullRobu)
    {
      ...
      robuMdl <- robu.custom(
        ...
        userdfs      = k_RVEModel$REModels[["overall"]]$reg_table$dfs
      )
      
      mat[moderator, class] <- round(robuMdl$reg_table$prob[2],6)
    }
  }
}
