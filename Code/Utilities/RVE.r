...
)
{
  ...
  if (!is.null(measureSub)) {
    data$Meas <- data$Meas - data[[measureSub]]
  }
...
) 
{
  ...
  if (is.null(extraCol)) {
    ...
    ilab.xpos <- c(-1.9, -1.2) * measScale
    ilab.pos <- 2
    xlim <- c(-5, 2.5) * measScale
    colLabels <- c("N", "Weight", paste(effSizeName, "[95% CI]"))
  } else {
    ilab = cbind(data$n, format(round(data$weight,2), trim = F), data$extra)
    ilab.xpos <- c(c(-1.9, -1.2) * measScale, 3 * measScale)
    ilab.pos <- 2
    xlim <- c(-5, 3) * measScale
  ...
  forest.col(
    ...
    slab.col = slab.col,
    anno.pos = anno.pos,
    refline = if (RVEModel$measureType == "p") c(-1, -0.5, 0, 0.5, 1)*measScale else 0
  )
...
)
{
  ...
  if (trimnfill)
  {
    in_standErr <- data$standErr[data$published]
    in_Meas <- data$Meas[data$published]
    if (zTrans)
    {
      in_standErr <- (tanh(data$Meas[data$published] + data$standErr[data$published]) - tanh(data$Meas[data$published] - data$standErr[data$published])) / 2
      in_Meas <- tanh(data$Meas[data$published])
    }
